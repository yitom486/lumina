...
impl MediaLibraryService {
    ...
    /// Agent calls go through the app-injected `agent` (M7).
    pub fn resolve_preview(
        &self,
        root: String,
        group_key: String,
        config: ResolverRunConfig,
        agent: &dyn lumina_core::AgentInvoker,
    ...
    pub fn group_label_for_media(
        ...
        let relative = crate::paths::relativize_under_root(&root, &media_path_buf).ok()?;
        let file = index
            .files
            .iter()
    ...
    pub fn series_for_media(
        ...
        if group.kind != crate::model::MediaGroupKind::Series {
            return Ok(None);
        }
        let label = group
            .manual_title
...
mod tests {
    ...
    fn first_scan_writes_and_reloads_local_index() {
        ...
        assert_eq!(stored.groups[0].kind, crate::model::MediaGroupKind::Series);
