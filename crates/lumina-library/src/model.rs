...
pub struct LibraryScanIssue {
    pub code: crate::error::LibraryErrorCode,
    pub message: String,
}
...
pub enum LibraryScanEvent {
    ...
    Failed {
        code: crate::error::LibraryErrorCode,
        message: String,
    },
}
...
pub enum ResolverProviderConfig {
    AcpAgent {
        profile_id: String,
        /// Opaque agent profile selection payload. The wire shape matches the
        /// app's ACP hint JSON; only the app interprets it (M7).
        profiles: Value,
        #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct AgentModelDiscoveryConfig {
    ...
    /// Opaque agent profile selection payload (see `ResolverProviderConfig`).
    pub profiles: Value,
}

/// Library-owned short-lived discovery result. JSON shape mirrors the agent
/// backend result; the app adapter converts (M7).
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
...
/// Library-owned model options for agent discovery results.
#[derive(Debug, Clone, Default, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct AgentModelOptions {
    pub models: Vec<AgentModelOption>,
    pub reasoning_efforts: Vec<AgentModelOption>,
    pub current_model_id: Option<String>,
    pub current_reasoning_effort: Option<String>,
}

/// Library-owned short-lived discovery result (no agent types inside).
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct AgentModelDiscoveryResult {
    pub connected: bool,
