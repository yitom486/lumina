...
use crate::{resolver, store, wikipedia};
...
pub fn load_context(
    ...
    let media_type = match group.resolution {
        crate::model::GroupResolution::Matched { media_type, .. } => media_type,
        _ => return Ok(None),
    };
...
mod tests {
    use super::*;
    use crate::model::{WikiCharacter, WikiEpisodeSummary, WikiMatchInfo};
    use serde_json::json;
