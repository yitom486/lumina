...
fn tool_json(name: &str) -> Option<Value> {
    ...
    if name == contract::TOOL_SEEK_PLAYBACK {
        let mut tool = json!({
            ...
            "description": "Request the player to seek to a given time (milliseconds) after you have located the moment via transcript/library tools. The request is confirmed by the host player; the result reports the actual landed position. Only use timestamps that tool results actually returned — never invent a time. Greeting or unrelated chat must not trigger this tool.",
            "inputSchema": {
                "type": "object",
                "properties": {
...
mod tests {
    ...
    fn tool_list_matches_canonical_contract() {
        ...
        for name in contract::ALL_TOOLS {
            ...
            // Concurrency signal: subtitle writer mutates disk; seek control
            // drives the host player — both stay out of the parallel partition.
            let read_only = tool
    ...
    fn initialize_exposes_stable_instructions_only_for_tool_profiles() {
        ...
        // Natural-reply rule: system instructions are applied silently, the
        // reply always faces the user's actual question.
        assert!(instructions.contains("系统说明"));
        assert!(instructions.contains("禁止在回复中提及"));
        assert!(instructions.contains("如同没有看过任何系统说明"));
