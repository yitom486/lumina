...
pub fn find_cli_any() -> Option<PathBuf> {
    ...
    // Dev workspaces: crate manifest no longer sits next to `native/`
    // (M7); reuse the shared ancestor lookup, then `native/ytdl/`.
    for root in lumina_media::tools::workspace_native_roots() {
        candidates.push(root.join("ytdl").join(CLI_NAME));
    }
